...
/** 解析项目根：config 显式值 > 从 base 向上找最近的 project.godot > base。 */
function resolveProjectRoot(explicit, base) {
...
function normalizeConfig(config) {
	return {
		// 项目根：config 显式值优先；否则从 dsh 工作目录向上找最近的 project.godot；
		// 仍找不到就用工作目录本身（错误日志会打印实际取值）
		projectRoot: resolveProjectRoot(config?.projectRoot, process.cwd()),
...
export function apply(ctx, config) {
	...
	async function startBuild() {
		...
		st.preset = preset;
		if (!preset) {
			const cfgPath = path.join(cfg.projectRoot, "export_presets.cfg");
			st.log.append("✘ 找不到可用的 Web 导出预设。\n");
			if (!fs.existsSync(cfgPath)) {
				st.log.append(`  已检查：${cfgPath}（文件不存在）\n`);
				st.log.append(`  当前 projectRoot=${cfg.projectRoot}（默认取 dsh 进程工作目录）\n`);
				st.log.append("  若目标项目在其它目录：请在 cordis.patch.yml 的 config 里设置 projectRoot（或从项目目录启动 dsh web），然后重启。\n");
			} else {
