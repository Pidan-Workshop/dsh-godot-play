 ...
 * webServer / subprocess / webRuntime / workspaceRegistry 四个宿主服务。
 * 注：workspaceRegistry 是 dsh-workspace 提供的服务名（ctx.workspaceRegistry），
 * 其在 bundle 加载器里的行 id 是 workspace，服务名并非 workspace。
 *
 ...
 *   - POST /api/godot-play/logs/clear  → 清空内存日志尾环
...
/** 宿主服务依赖。 */
export const inject = ["webServer", "subprocess", "webRuntime", "workspaceRegistry"];
...
function normalizeConfig(config) {
	return {
		...
		// 导出输出相对目录：留空（""）= 自动跟随 export_presets.cfg 里 Web 预设的 export_path 目录；
		// 显式填写则覆盖自动探测（兼容旧行为）。解析不出时兜底 <项目>/web。
		webRel: config?.webRel ?? "",
...
async function resolveGodot(ctx, cfg) {
	for (const cand of godotCandidates(cfg)) {
		...
		try {
			// dsh-subprocess 的 resolveExecutable 是 async（返回 Promise<路径>），
			// 必须 await——不 await 拿到的是 truthy 的 Promise 对象，塞进 argv 后 .includes 直接炸。
...
/** 解析 export_presets.cfg，返回全部预设的 {index, name, platform, runnable, exportPath}。 */
function parsePresets(projectRoot) {
...
export function detectWebPreset(projectRoot, override) {
	...
	const web = parsePresets(projectRoot).filter((p) => p.platform === "Web" && p.name);
	if (web.length === 0) return null;
	return (web.find((p) => p.runnable) ?? web[0]).name;
}

/** 从 export_presets.cfg 里读 Web 预设（优先 runnable，尊重 override）的 export_path，
 *  返回其目录（相对项目根，去 res:// 前缀）。仅接受"项目内子目录"形态的相对路径；
 *  解析不出或路径不安全（根目录/上级逃逸/绝对路径）时返回 null（上层兜底 webRel / "web"）。 */
export function detectWebExportDir(projectRoot, override) {
	const web = parsePresets(projectRoot).filter((p) => p.platform === "Web" && p.name);
	if (web.length === 0) return null;
	const pick = override
		? (web.find((p) => p.name === override) ?? web[0])
		: (web.find((p) => p.runnable) ?? web[0]);
	if (!pick?.exportPath) return null;
	let rel = pick.exportPath.trim();
	if (rel.startsWith("res://")) rel = rel.slice("res://".length);
	if (!rel || path.isAbsolute(rel)) return null;
	rel = path.posix.normalize(rel).replace(/\\/g, "/");
	const dir = path.posix.dirname(rel);
	if (!dir || dir === "." || dir.startsWith("..") || dir.includes("/../") || dir === "/") return null;
	return dir;
}
...
export function apply(ctx, config) {
	...
	/** 当前目标的导出输出目录（每次实时解析，跟随目标项目切换）：
	 *  显式 cfg.webRel（非空）> 跟随 Web 预设 export_path 的目录 > 兜底 <项目>/web。 */
	function currentWebRoot(root = currentRoot()) {
		if (cfg.webRel) return path.join(root, cfg.webRel);
		const dir = detectWebExportDir(root, cfg.exportPreset);
		return dir ? path.join(root, dir) : path.join(root, "web");
	}
	...
	async function startBuild() {
		...
		const webRoot = currentWebRoot(root);
		...
		const godot = await resolveGodot(ctx, cfg);
		st.godot = godot;
		if (!godot) {
	...
	function handleBuild(req, res) {
		...
		void startBuild().catch((err) => {
			st.log.append("✘ 构建意外中断：" + (err?.message ?? String(err)) + "\n");
			st.state = "error";
			st.finishedAt = Date.now();
		});
	...
	function handleStatus(req, res) {
		...
		sendJson(res, 200, {
			...
			webReady: fs.existsSync(path.join(currentWebRoot(), "index.html")),
			godot: st.godot,
			preset: st.preset,
	...
	ctx.effect(() => {
		const disposers = [
			...
			ctx.webServer.register({ kind: "exact", path: `${API_BASE}/logs/clear`, handler: handleLogsClear }),
