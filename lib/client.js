...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apiClearLog() {
			return fetchJson(API_BASE + "/logs/clear", { method: "POST", cache: "no-store" });
		}
		...
		/** 清空日志：宿主内存尾环 + 屏幕显示一起清（构建中也可清，新输出照常追加）。 */
		function clearLog() {
			if (!ui) return;
			apiClearLog().then(function () {
				setLog("");
			}).catch(function () {
				setChip("error", "⚠️ 清空日志请求失败（宿主未就绪？）", true);
			});
		}

		/** 重新启动当前游戏：带缓存戳重载 iframe（不触发重新构建）。 */
		function restartGame() {
			if (!ui) return;
			if (!ui.frame) { ensureFrame(); if (!ui.frame) return; }
			var busy = addBusy("重启中…");
			var remove = function () {
				if (busy.parentNode === ui.frameWrap) ui.frameWrap.removeChild(busy);
			};
			ui.frame.addEventListener("load", remove);
			// 兜底：加载出错导致 load 不触发时也收掉遮罩
			setTimeout(function () {
		...
		function mountUi() {
			...
			style.textContent = [
				...
				"#" + ROOT_ID + " .log-wrap[hidden] { display: none; }",
				...
				"#" + ROOT_ID + " .busy { position: absolute; inset: 0; z-index: 1; display: flex; align-items: center; justify-content: center; padding: 0 12px; color: #9aa3ad; font-size: 13px; text-align: center; background: #14181c; }",
			...
			root.innerHTML = [
				...
				'    <button class="btn-minor restart" type="button" title="重新加载当前游戏（不重新构建）">↻ 重新启动</button>',
				'    <button class="btn-minor log-toggle" type="button">日志</button>',
				'    <button class="btn-minor log-clear" type="button" title="清空构建日志">清空日志</button>',
				'  </div>',
			...
			root.querySelector(".restart").addEventListener("click", restartGame);
			root.querySelector(".log-toggle").addEventListener("click", toggleLog);
			root.querySelector(".log-clear").addEventListener("click", clearLog);
		}
